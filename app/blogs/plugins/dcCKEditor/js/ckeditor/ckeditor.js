...
﻿
(function(){if(window.CKEDITOR&&window.CKEDITOR.dom)return;if(!window.CKEDITOR){window.CKEDITOR=(function(){var basePathSrcPattern=/(^|.*[\\\/])ckeditor\.js(?:\?.*|;.*)?$/i;var CKEDITOR={timestamp:'F63K',version:'4.5.1',revision:'0',rnd:Math.floor(Math.random()*(999-100+1))+100,_:{pending:[],basePathSrcPattern:basePathSrcPattern},status:'unloaded',basePath:(function(){var path=window.CKEDITOR_BASEPATH||'';if(!path){var scripts=document.getElementsByTagName('script');for(var i=0;i<scripts.length;i++){var match=scripts[i].src.match(basePathSrcPattern);if(match){path=match[1];break;}}}
...
element=this.getParent();if(!checkOnlyAttributes&&element&&typeof element.$.isContentEditable!='undefined')
return!(element.$.isContentEditable||element.data('cke-editable'));else{while(element){if(element.data('cke-editable')){return false;}else if(element.hasAttribute('contenteditable')){return element.getAttribute('contenteditable')=='false';}
...
while(nextSibling){if(nextSibling.equals(endNode)){lastConnectedLevel=level;break;}
if(nextSibling.equals(endParents[level])){lastConnectedLevel=level;break;}
...
eventData='';}
eventData={dataValue:eventData};!internal&&this.fire('getData',eventData);return eventData.dataValue;},getSnapshot:function(){var data=this.fire('getSnapshot');if(typeof data!='string'){var element=this.element;if(element&&this.elementMode==CKEDITOR.ELEMENT_MODE_REPLACE)
...
range.moveToBookmark(that.targetBookmark);range.optimize();helpers.fixUneditableRangePosition(range);helpers.list.merge(that,this);helpers.table.purge(that,this);helpers.block.merge(that,this);if(removeEmptyBlock){var path=range.startPath();if(range.checkStartOfBlock()&&range.checkEndOfBlock()&&path.block&&!range.root.equals(path.block)){range.moveToPosition(path.block,CKEDITOR.POSITION_BEFORE_START);path.block.remove();}}else{helpers.autoParagraph(this.editor,range);if(isEmpty(range.startContainer))
...
function initPasteClipboard(editor){var clipboard=CKEDITOR.plugins.clipboard,preventBeforePasteEvent=0,preventPasteEvent=0,inReadOnly=0,mainPasteEvent=CKEDITOR.env.ie?'beforepaste':'paste';addListeners();addButtonsCommands();editor.getClipboardData=function(options,callback){var beforePasteNotCanceled=false,dataType='auto',dialogCommited=false;if(!callback){callback=options;options=null;}
...
editable.on(mainPasteEvent,function(evt){if(CKEDITOR.env.ie&&preventBeforePasteEvent)
...
function getClipboardDataDirectly(){if(!CKEDITOR.env.ie){editor.fire('beforePaste',{type:'auto',method:'paste'});return false;}
editor.focus();preventPasteEventNow();var focusManager=editor.focusManager;focusManager.lock();if(editor.editable().fire(mainPasteEvent)&&!execIECommand('paste')){focusManager.unlock();return false;}
...
function pasteDataFromClipboard(evt){var eventData={type:'auto',method:'paste',dataTransfer:clipboard.initPasteDataTransfer(evt)},htmlInExternalDataTransfer=!CKEDITOR.env.ie&&!CKEDITOR.env.safari,external=eventData.dataTransfer.getTransferType(editor)===CKEDITOR.DATA_TRANSFER_EXTERNAL;eventData.dataTransfer.cacheData();var beforePasteNotCanceled=editor.fire('beforePaste',eventData)!==false;if(beforePasteNotCanceled&&(htmlInExternalDataTransfer||!external)&&!eventData.dataTransfer.isEmpty()){evt.data.preventDefault();setTimeout(function(){firePasteEvents(editor,eventData);},0);}else{getClipboardDataByPastebin(evt,function(data){eventData.dataValue=data.replace(/<span[^>]+data-cke-bookmark[^<]*?<\/span>/ig,'');beforePasteNotCanceled&&firePasteEvents(editor,eventData);});}}
...
isDropRangeAffected=this.isDropRangeAffectedByDragRange(dragRange,dropRange);if(!isDropRangeAffected){dragBookmark=dragRange.createBookmark(1);}
dropBookmark=dropRange.clone().createBookmark(1);if(isDropRangeAffected){dragBookmark=dragRange.createBookmark(1);}
...
return dataTransfer;}else{return new this.dataTransfer(null,sourceEditor);}}};var clipboardIdDataType=CKEDITOR.plugins.clipboard.isCustomDataTypesSupported?'cke/id':'Text';CKEDITOR.plugins.clipboard.dataTransfer=function(nativeDataTransfer,editor){if(nativeDataTransfer){this.$=nativeDataTransfer;}
this._={chromeLinuxRegExp:/^<meta.*?>/,chromeWindowsRegExp:/<!--StartFragment-->([\s\S]*)<!--EndFragment-->/,data:{},files:[],normalizeType:function(type){type=type.toLowerCase();if(type=='text'||type=='text/plain'){return'Text';}else if(type=='url'){return'URL';}else{return type;}}};this.id=this.getData(clipboardIdDataType);if(!this.id){if(clipboardIdDataType=='Text'){this.id='';}else{this.id='cke-'+CKEDITOR.tools.getUniqueId();}}
...
if(type=='text/html'&&CKEDITOR.env.chrome){data=data.replace(this._.chromeLinuxRegExp,'');result=this._.chromeWindowsRegExp.exec(data);if(result&&result.length>1){data=result[1];}}
...
return;editor.addCommand(pluginName,horizontalruleCmd);editor.ui.addButton&&editor.ui.addButton('HorizontalRule',{label:editor.lang.horizontalrule.toolbar,command:pluginName,toolbar:'insert,40'});}});})();(function(){CKEDITOR.plugins.add('image',{requires:'dialog',init:function(editor){if(editor.plugins.image2)
...
if(isAtEnd&&next){nextLine=range.clone();nextLine.moveToElementEditStart(next);if(isAtEnd==2){cursor.moveToPosition(cursor.endPath().block,CKEDITOR.POSITION_BEFORE_END);if(nextLine.endPath().block){nextLine.moveToPosition(nextLine.endPath().block,CKEDITOR.POSITION_AFTER_START);}}
...
return limit;else
return null;return trigger;}else{return null;}}
return null;}
...
desc.remove();});iframe.setAttributes({tabIndex:editor.tabIndex,allowTransparency:'true'});!useOnloadEvent&&onLoad();if(CKEDITOR.env.webkit){var onResize=function(){contentSpace.setStyle('width','100%');iframe.hide();iframe.setSize('width',contentSpace.getSize('width'));contentSpace.removeStyle('width');iframe.show();};iframe.setCustomData('onResize',onResize);CKEDITOR.document.getWindow().on('resize',onResize);}
...
setTimeout(function(){editor.fire('dataReady');},0);},0,this);}
